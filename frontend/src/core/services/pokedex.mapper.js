/**
 * Normalize a raw API pokemon object (camelCase, _id-keyed) into the shape
 * the UI expects. Shared by the list view and the detail view so both pages
 * apply the exact same transformation.
 */
export const mapPokemon = (item) => {
  if (!item) return null;
  ...
  return {
    ...
    description:
      Array.isArray(item.flavorTextEntries) && item.flavorTextEntries.length > 0
 ...
 */
export const mapPokemonSummary = (item) => {
  if (!item) return null;
  return {
    id: item._id,
    name: item.name,
